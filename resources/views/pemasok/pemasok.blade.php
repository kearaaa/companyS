<x-app-layout>
  ...
  <dialog id="my_modal_3" class="modal">
    <div class="modal-box">
      ...
      <form action="{{ route('pemasok') }}" method="post" autocomplete="off">
        @csrf
        <h2 class="text-2xl font-bold mb-4">Input Pemasok</h2>
        <div class="mb-4">
          ...
          <input type="text" id="nama_pemasok" name="nama_pemasok" required
            class="border rounded-md py-2 px-3 w-full focus:outline-none focus:ring focus:border-blue-300 bg-slate-500 text-white" value="{{ old('nama_pemasok') }}">
          @error('nama_pemasok')
        ...
        <div class="mb-4">
          ...
          <input type="email" id="email" name="email" required
            class="border rounded-md py-2 px-3 w-full focus:outline-none focus:ring focus:border-blue-300 bg-slate-500 text-white" value="{{ old('email') }}">
            @error('email')
            <p class="text-red-500 text-sm">{{ $message }}</p>
          @enderror
        </div>
        ...
        <div class="mb-4">
          ...
          <select id="id_produk" name="id_produk" required
            ...
            @foreach ($produk as $p)
              <option value="{{ $p->id_produk }}"  @if (old('id_produk') == $p->id_produk) selected @endif>{{ $p->nama_produk }}</option>
            @endforeach
          </select>
        </div>
              ...
              @foreach ($pemasok as $data)
                <tr class="border-b">
                  ...
                  <td class="space-x-2">
                    ...
                    <a href="{{ route('pemasok.edit', $data->id_pemasok) }}"
                      class="text-blue-500 hover:underline">Edit</a>
                    <a href="{{ route('pemasok.destroy', $data->id_pemasok) }}" class="text-red-500 hover:underline"
