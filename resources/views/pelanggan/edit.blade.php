    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900 dark:text-gray-100">
          <form action="{{ route('pelanggan.update', $pelanggan->id_pelanggan) }}" method="post" autocomplete="off">
            @csrf
            @method('PUT')
            <h2 class="text-2xl font-bold mb-4">Edit Pelanggan</h2>
            <div class="mb-4">
              <label for="nama_pelanggan" class="block text-sm font-medium text-white">Nama Pelanggan:</label>
            ...
            <div class="mb-4">
              ...
              <textarea type="text" id="alamat" name="alamat" required
                class="border rounded-md py-2 px-3 w-full focus:outline-none focus:ring focus:border-blue-300 bg-slate-500 text-white">{{ $pelanggan->alamat }}</textarea>
            </div>

            <div class="mb-4">
              <label for="email" class="block text-sm font-medium text-white">Email:</label>
              <input type="email" id="email" name="email" required value="{{ $pelanggan->email }}"
                class="border rounded-md py-2 px-3 w-full focus:outline-none focus:ring focus:border-blue-300 bg-slate-500 text-white">
            </div>
            ...
            <a href="/pelanggan" class="btn">back</a>
          </form>
        </div>
      </div>
    </div>
